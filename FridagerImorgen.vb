Sub Main(ByVal deviceReference As String)
        'Moskus 2017
		hs.WriteLog( "Info", "FridagerImorgen: Starter script")
        Dim devRef As Integer = Integer.Parse(deviceReference.ToString)
		Dim dateToCheck=DateAdd("d",1,Now)
		hs.WriteLog( "Info", "FridagerImorgen: Behandler dato: " & Year(dateToCheck) & "-" & Month(dateToCheck) & "-" & Day(dateToCheck) )
        Dim isCurrentDateHoliday As Boolean = False
        If Not isCurrentDateHoliday Then isCurrentDateHoliday = IsOfficialHoliday(dateToCheck) 'Sjekker offisielle helligdager
        If Not isCurrentDateHoliday Then isCurrentDateHoliday = IsCustomHoliday(dateToCheck)   'Sjekker egen-spesifiserte dager

        If isCurrentDateHoliday Then
            hs.CAPIControlHandler(hs.CAPIGetSingleControlByUse(devRef, HomeSeerAPI.ePairControlUse._On))
			hs.WriteLog( "Info", "FridagerImorgen: Satte til fridag")
        Else
            hs.CAPIControlHandler(hs.CAPIGetSingleControlByUse(devRef, HomeSeerAPI.ePairControlUse._Off))
			hs.WriteLog ("Info", "FridagerImorgen: Satte til arbeidsdag")
        End If
